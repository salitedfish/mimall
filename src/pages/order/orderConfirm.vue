<template>
  <div class="order-confirm">
        ...
        <div class="order-box">
          ...
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" v-for="(item, index) in list" :key="index" :class="{'checked': index == checkedIndex }" @click="addressChecked(index)">
            ...
            <ul>
              <li v-for="(item, index) in cartList" :key="index" >
                ...
                <div class="good-price">
                  {{ `${item.productPrice} × ${item.quantity} 万` }}
                </div>
                <div class="good-total">{{ item.productTotalPrice }} 万</div>
              </li>
            </ul>
          </div>
          <!----------------- 以下为确认结果区域 ---------------->
          ...
          <div class="detail">
            <div class="item">
              ...
              <span class="item-val">{{ checkedTotalQuantity }}件</span>
            </div>
            <div class="item">
              <span class="item-name">商品总价：</span>
              <span class="item-val">{{ cartTotalPrice }}万</span>
            </div>
            <div class="item">
              <span class="item-name">优惠活动：</span>
              <span class="item-val">0万</span>
            </div>
            <div class="item">
    ...
    >
      <!-- 下面是删除的modal框 -->
      <template v-slot:modal-body v-if="userAction == 0">
        <p>您确认要删除此地址吗</p>
      </template>
      <!-- 下面是新增的modal框 -->
      <template v-slot:modal-body v-else-if="userAction == 1 || userAction == 2">
        <div class="edit-wrap">
          <div class="item">
            <input
              type="text"
...
export default {
  ...
  data() {
    return {
      ...
      cartTotalPrice: 0,
      checkedTotalQuantity: 0,
      checkedItem: {}, //保存传入的地址项
      userAction: "", //保存用户动作0,1,2
      showModal: false,
      title: "",
      checkedIndex: 0,
    };
  ...
  methods: {
    ...
    getCartList() {
      axios.get("/carts").then((res) => {
        ...
        for (let item of this.cartList) {
          this.checkedTotalQuantity += item.quantity;
        }
      });
    },
    ...
    //更改地址
    changeAddress(item) {
      this.checkedItem = {...item};//这里必须要深克隆，不然更改表单数据时，页面上的地址也会实时更改，但是后台并没有更新
    ...
    submitAddress() {
      ...
      axios[method](url, checkedItem).then(() => {
        this.closeModal();
        this.getAddressLIst();
